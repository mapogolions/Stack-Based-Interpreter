...
object Parse {
  ...
  def commands(xs: List[String]): List[Commands] = xs match {
    ...
    case line :: ls => line.split(' ').toList match {
      ...
      case "rem" :: t     => REM :: commands(ls)
      case "swap" :: t    => SWAP :: commands(ls)
      case "if" :: t      => IF :: commands(ls)
      case "and" :: t     => AND :: commands(ls)
      case "or" :: t      => OR :: commands(ls)
      case "neg" :: t     => NEG :: commands(ls)
      case "equal" :: t   => EQUAL :: commands(ls)
      case "lessThan" :: t => LESSTHAN :: commands(ls)
      case "not" :: t     => NOT :: commands(ls)
